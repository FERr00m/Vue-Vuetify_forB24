...
export default class BX24Wrapper {
  ...
  constructor() {
    ...
    /**
     * Функция для подсчета количесва задач
     * @type {function}
     */
    this.counterTasks = (counter) => {};
    ...
    return new Promise((resolve, reject) => {
      let callback = (result) => {
        ...
        if (result.status != 200 || result.error()) {
          return reject(
            `${result.error()} (callMethod ${method}: ${JSON.stringify(
              params
            )})`
          );
        }
  ...
  async callListMethod(method, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let callback = async (result) => {
        ...
        if (result.status != 200 || result.error()) {
          return reject(
            `${result.error()} (callListMethod ${method}: ${JSON.stringify(
              params
            )})`
          );
        }
        ...
        this.progress(
          total > 0 ? Math.round((100 * data.length) / total) : 100
        );
  ...
  async *fetchList(method, params = {}) {
    ...
    params.filter = params.filter || { '>ID': 0 };
    params.start = -1;
  ...
  async *fetchTaskList(method, params = {}) {
    ...
    do {
      ...
      params.filter['>ID'] = data.tasks[data.tasks.length - 1].id;
    } while (true);
  }
      ...
      let callback = (results) => {
        ...
        if (Array.isArray(results)) {
          ...
          for (let result of results) {
            if (result.status != 200 || result.error()) {
              return reject(
                `${result.error()} (callBatch ${result.query.method}: ${
                  result.query.data
                })`
              );
            }
  ...
  throttleCall() {
    return new Promise((resolve) => {
      let timeout = Math.round(
        this.lastRequestTime + 1e3 * (1 / this.throttle) - Date.now()
      );
      if (timeout <= 0) {
